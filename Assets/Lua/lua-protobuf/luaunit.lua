    ...
    if mt and mt.__tostring then
        ...
        for k, v in sortedPairs(tbl) do
            ...
            if recursionTable[v] then
                ...
                entry = entry .. prettystr_sub(v, indentLevel + 1, printTableRefs, recursionTable)
            end
            count = count + 1
            result[count] = entry
        end
...
function M.assertStrContains(str, sub, useRe)
    ...
    if not string.find(str, sub, 1, not useRe) then
        ...
        fail_fmt(2, "Error, %s %s was not found in string %s", useRe and "regexp" or "substring", sub, str)
    end
end
...
function M.assertStrIContains(str, sub)
    ...
    if not string.find(str:lower(), sub:lower(), 1, true) then
        sub, str = prettystrPairs(sub, str, "\n")
        fail_fmt(2, "Error, substring %s was not found (case insensitively) in string %s", sub, str)
    end
end
...
function M.assertNotStrContains(str, sub, useRe)
    ...
    if string.find(str, sub, 1, not useRe) then
        sub, str = prettystrPairs(sub, str, "\n")
        fail_fmt(2, "Error, %s %s was found in string %s", useRe and "regexp" or "substring", sub, str)
    end
end
...
function M.assertNotStrIContains(str, sub)
    ...
    if string.find(str:lower(), sub:lower(), 1, true) then
        sub, str = prettystrPairs(sub, str, "\n")
        fail_fmt(2, "Error, substring %s was found (case insensitively) in string %s", sub, str)
    end
end
...
function M.assertStrMatches(str, pattern, start, final)
    ...
    if not strMatch(str, pattern, start, final) then
        ...
        fail_fmt(2, "Error, pattern %s was not matched by string %s", pattern, str)
    end
end
...
for _, funcName in ipairs(
    ...
    M[funcName] = function(value)
        if type(value) == typeUnexpected then
            fail_fmt(2, "Not expected: a %s type, actual: value %s", typeUnexpected, prettystrPairs(value))
        end
    end
end
...
function M.assertNotIs(actual, expected)
    if actual == expected then
        ...
        fail_fmt(2, "Expected object and actual object are the same object: %s", prettystrPairs(expected))
    end
end
